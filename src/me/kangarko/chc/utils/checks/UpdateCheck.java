...
public class UpdateCheck implements Runnable {
	...
	@Override 
	public void run() {
		String staraVer = ChatControl.instance().getDescription().getVersion();
		...
		if (cislom(novaVerzia) > cislom(staraVer)) {
			if (Settings.Updater.DOWNLOAD) {
				...
				Variables.newVersion = novaVerzia;
				Common.Log("&eA new version of ChatControl is available!");
				Common.Log("&eCurrent version: " + ChatControl.instance().getDescription().getVersion() + " New version: " + novaVerzia);
			}
		}
	}
